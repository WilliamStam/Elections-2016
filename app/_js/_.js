...
var toolbar = [['Source'], ['Cut', 'Copy', 'Paste', 'Find', 'Replace'], ['Bold', 'Italic', 'Underline', 'StrikeThrough'], ['Styles', 'Format', 'Font', 'FontSize'], ['NumberedList', 'BulletedList'], ['Outdent', 'Indent']];
var toolbar_small = [['Source'], ['Cut', 'Copy', 'Paste', 'Find', 'Replace'], ['Bold', 'Italic', 'Underline', 'StrikeThrough']];
var ckeditor_config = {
	height: '150px',
	toolbar: toolbar,
...
//	console.log("nbbfs:"+nbbfs+" | s:"+s+" | currentScroll:"+currentScroll+" | fs:"+fs+" | p:"+p+" | fs_mt:"+fs_mt)
		...
		$(".navbar-brand").css({
			"font-size": fs, "margin-bottom": -fs, "margin-top": fs_mt,
		})
	} else {
		
		$(".navbar-brand").css({
			"font-size": 34, "margin-bottom": -34, "margin-top": 2,
		})
	}
	...
	$.getData("/iec/voter?IDNumber=" + key, function (data) {
		...
		if (data.geojson) {
			$.doTimeout(400, function () {
				...
				if (data.Voter.VotingStation) {
					if (data.Voter.VotingStation.Location.Latitude && data.Voter.VotingStation.Location.Longitude) {
						...
						var LeafIcon = L.Icon.extend({
							options: {
								...
								iconSize: [100, 62], iconAnchor: [50, 62],
						...
						L.marker([data.Voter.VotingStation.Location.Latitude, data.Voter.VotingStation.Location.Longitude], {icon: greenIcon}).addTo(mapm);
					}
				}
